...
class MainView(tk.Frame):
    ...
    def montar_tela(self):
        ...
        btn_sair = tk.Button(nav_bar, text="Sair do Sistema", bg="#ef4444", fg="white", font=("Segoe UI", 9, "bold"), 
                             command=self.confirmar_saida, bd=0, cursor="hand2", activebackground="#dc2626", activeforeground="white", padx=15)
        ...
        # Popula os dados de todas as abas assim que a tela principal abre
        # (cada aba já constrói sua própria UI sozinha no __init__; aqui só
        # carregamos os dados nela, e o mesmo é refeito a cada troca de aba
        # para refletir alterações feitas em outras abas).
        self.atualizar_todas_abas()
        self.notebook.bind("<<NotebookTabChanged>>", lambda e: self.atualizar_todas_abas())

    def confirmar_saida(self):
        if messagebox.askyesno("Sair do Sistema", "Tem certeza que deseja sair e voltar para a tela de login?"):
            self.on_logout()
