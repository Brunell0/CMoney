...
class AuthView(tk.Frame):
    ...
    def acao_login(self):
        sucesso, mensagem = self.controller.login(self.ent_user.get(), self.ent_pass.get())
        if sucesso:
            self.on_success()
        else:
    ...
    def acao_registro(self):
        if not self.ent_user.get() or not self.ent_pass.get():
            ...
            return
        sucesso, mensagem = self.controller.registrar_usuario(self.ent_user.get(), self.ent_pass.get(), self.cb_perfil.get())
        if sucesso:
