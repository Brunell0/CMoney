...
class TransactionController:
    ...
    def atualizar_transacao(self, id_t: int, tipo: str, descricao: str, valor: float,
        ...
        if self.__dbC.checa_transacao_existe(id_t): return False
        cat = self.__dbC.encontra_categoria(categoria_nome)
    ...
    def deletar_transacao(self, id_t: int, nome_usuario: str) -> tuple[bool, str]:
        if self.__dbC.checa_transacao_existe(id_t):
    ...
    def calcular_saldo(self) -> float:
        return sum(t.calcular_impacto() for t in self.__dbC.__db.__transacoes.values())

    def calcula_total_gasto(self, cat_nome) -> float:
       return sum(t.valor for t in self.__dbC.__db.__transacoes.values() 
            if t.__class__.__name__ == "Despesa" and t.categoria == cat_nome)